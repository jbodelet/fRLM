...
fRLM_lpfr <- function(data, id, time, exposures, outcome, family="gaussian", controls=NULL, L=4, K=5, grid= seq(0,1,l=150), ...) {
  ...
  tobs <- lapply(data_split, function(di) di[time])
  xobs <- lapply(data_split, function(di) di[exposures])
  ...
  if (family == "gaussian") {
    stan_file = "./stan/lpfr.stan"
  } else if (family == "binary") {
    stan_file = "./stan/lpfr_binary.stan"
  }


  fit_list <- lpfr_stan(y = y, tobs=tobs, xobs=xobs, covariates=C, compiled_file =stan_file, L=L, K=K, grid=grid, ...)
